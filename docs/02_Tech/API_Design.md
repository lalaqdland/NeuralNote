...
|-----|------|------|------|
| **前端** | | | |
| Web | React | 18.x | 主Web应用 |
| 状态管理 | Redux Toolkit | 2.x | 全局状态管理 |
| 图谱可视化 | D3.js / Cytoscape.js | 7.x | 2D知识图谱 |
| 3D可视化 | Three.js + 3d-force-graph | 0.15.x | 3D知识图谱 |
| UI框架 | Ant Design / Material-UI | 5.x | 组件库 |
| **后端** | | | |
| 运行时 | Python | 3.11+ | 后端语言 |
| Web框架 | FastAPI | 0.109+ | API框架 |
| ORM | SQLAlchemy | 2.x | 数据库ORM |
| 异步任务 | Celery + Redis | 5.x | 异步任务队列 |
| **数据库** | | | |
| 主数据库 | PostgreSQL | 15.x | 关系型数据库 |
| 向量扩展 | PgVector | 0.5.x | 向量存储与搜索 |
| 缓存 | Redis | 7.x | 缓存与会话 |
| **AI/ML** | | | |
| OCR | 百度OCR + 腾讯OCR | - | 文字识别 |
| LLM | OpenAI GPT-4 / DeepSeek | - | 智能分析与解答 |
