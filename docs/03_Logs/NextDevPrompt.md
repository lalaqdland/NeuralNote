...
- **前端核心功能开发** ✅ 新增
  ...
  - 图谱详情页面（统计、可视化、节点列表）
- **前端功能完善** ✅ 新增（2026-02-01 15:30-18:00）
  - 节点关联管理组件（查看、创建、删除关联，6种关联类型）
  - 统计图表展示（Recharts - 5种图表类型）
  - 个人中心完善（信息编辑、密码修改、学习统计）
  - 向量搜索界面（智能搜索、参数配置、结果展示）
  - 全局搜索功能（导航栏集成）
- **3D 知识图谱可视化** ✅ 新增（2026-02-01 18:30-19:30）
  - Three.js 依赖安装（@react-three/fiber + @react-three/drei）
  - 3D 节点渲染（4种几何体、5级颜色、动态大小）
  - 3D 边渲染（关联关系可视化）
  - 4种 3D 布局算法（力导向、球形、螺旋、网格）
  - 交互功能（旋转、缩放、拖拽、点击、悬停）
  - 2D/3D 视图切换
  - 自动旋转模式
  - 控制面板和图例
- **复习提醒通知系统** ✅ 新增（2026-02-01 20:00-21:00）
  - 通知服务（权限管理、智能提醒、桌面通知）
  - 通知设置界面（权限状态、设置表单、通知历史）
  - 定时检查机制（可配置间隔、免打扰时间段）
  - 提示音功能（可选、音量控制）
  - 通知历史记录（最多100条、已读/未读标记）
  - 集成到主应用（导航栏入口、未读徽章）
  - 集成到复习页面（自动启动检查、URL参数支持）
- **数据导出功能** ✅ 新增（2026-02-01 21:00-21:30）
  - 导出服务（JSON/CSV/Markdown 三种格式）
  - 导出界面（格式选择、导出选项、加载状态）
  - JSON 导出（完整结构化数据）
  - CSV 导出（表格格式、Excel 兼容）
  - Markdown 导出（文档格式、按类型分组）
  - 集成到图谱详情页和列表页
